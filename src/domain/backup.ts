...
export function validateBackup(value: unknown): Backup {
  ...
  if (!reflections.every((item) => isObject(item) && isString(item.sessionId) && isString(item.createdAt) && isString(item.updatedAt))) throw new Error('A next-day reflection record is malformed.')
