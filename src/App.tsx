...
function MemoryMatch() {
  ...
  const clearMismatchTimer=()=>{if(mismatchTimer.current!==null){window.clearTimeout(mismatchTimer.current);mismatchTimer.current=null}}
  useEffect(()=>()=>clearMismatchTimer(),[])
  const restart=()=>{clearMismatchTimer();setDeck(makeMemoryDeck());setOpen([]);setMatched([]);setTurns(0);setLocked(false);setAnnouncement('New cards mixed. Choose any two cards to find a pair.')}
  const choose=(card:MemoryCard)=>{
    ...
    clearMismatchTimer();mismatchTimer.current=window.setTimeout(()=>{mismatchTimer.current=null;setOpen([]);setLocked(false);setAnnouncement('Cards closed. Choose another pair.')},reducedMotion?100:700)
  }
