...
function PlaySwitcher({ current }: { current: 'game' | 'room' }) {
  return <nav className="play-switcher" aria-label="Zanana play spaces"><Link className={current === 'game' ? 'active' : ''} aria-current={current === 'game' ? 'page' : undefined} to="/game"><Gamepad2 /> Arcade</Link><Link className={current === 'room' ? 'active' : ''} aria-current={current === 'room' ? 'page' : undefined} to="/game/room"><HomeIcon /> Zanana’s Room</Link></nav>
}
...
function SettingsPage() {
  const sessions=useLiveQuery(()=>db.sessions.toArray(),[])??[];const events=useLiveQuery(()=>db.events.toArray(),[])??[];const effects=useLiveQuery(()=>db.effects.toArray(),[])??[];const attachments=useLiveQuery(()=>db.attachments.toArray(),[])??[];const reflections=useLiveQuery(()=>db.reflections.toArray(),[])??[];const[theme,setTheme]=useState(localStorage.getItem('ht-theme')||'light');const[haptics,setHaptics]=useState(localStorage.getItem('ht-haptics')!=='off');const[reflectionPrompts,setReflectionPrompts]=useState(localStorage.getItem('ht-reflection-prompts')!=='off');const[importData,setImportData]=useState<ReturnType<typeof validateBackup>|null>(null);const[error,setError]=useState('');const[deleteStep,setDeleteStep]=useState(false);const[deleteText,setDeleteText]=useState('');const[status,setStatus]=useState('')
