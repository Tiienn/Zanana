---
name: finn-build
description: Claim the next safe agent-ready issue from Linear, implement it, and open a PR. Use when asked to run Finn-loop's builder, work the approved queue, or fix Finn-loop review feedback. One invocation performs one unit of work; repeat manually or with a Codex Scheduled task.
---

# Finn-loop builder

One pass = one unit of work: fix review feedback on one existing PR, or build
one issue end to end. In Codex, invoke this skill as `$finn-build`. To keep it
running, repeat it manually or create a Codex Scheduled task whose durable
prompt invokes `$finn-build` once per run. Never run more than one builder for
the same Linear team at a time.
...
Using the Linear connector, list issues on team `ZAN` that meet every
condition:
...
- Fetch the latest default branch from `origin` and create or resume a branch
  named `ZAN-NNN-short-slug`, using the issue's real identifier.
...
- `Closes ZAN-NNN`, using the real Linear issue identifier
