---
name: finn-review
description: Review open PRs against their linked Linear issues and required GitHub checks, then post a three-group verdict with Finn-loop labels. Use when asked to run Finn-loop's reviewer or review its PR queue. One invocation reviews at most one PR; never merges or pushes code.
---

# Finn-loop reviewer

One pass = one PR reviewed. In Codex, invoke this skill as `$finn-review`.
To keep reviews running, repeat it manually or create a Codex Scheduled task
whose durable prompt invokes `$finn-review` once per run.
...
- Parse the linked issue identifier from `Closes ZAN-NNN` in the PR body and
